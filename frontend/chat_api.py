    ...
    for preset_name, config_path_str in PRESET_CONFIG_PATHS.items():
        try:
            ...
            preset_configs[preset_name] = {
                ...
                # Safely get this key, default to False
                "normalize": config["vector"].get("normalize", False),
                "model_path": config["model"]["path"]
            }
            logger.info(
                f"Successfully loaded config for preset: {preset_name} from {config_path_str}")
        except Exception as e:
            logger.error(
                f"Failed to load config file {config_path_str} for preset {preset_name}: {str(e)}")
...
def chat():
    ...
    try:
        ...
        if preset not in preset_configs:
            logger.warning(
                f"No config found for preset: {preset}. Using dummy response.")
