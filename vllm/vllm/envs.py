...
if TYPE_CHECKING:
    ...
    VLLM_USE_RAY_COMPILED_DAG_CHANNEL_TYPE: Literal["auto",
                                                    "nccl", "shm"] = "auto"
    ...
    CMAKE_BUILD_TYPE: Literal["Debug", "Release",
                              "RelWithDebInfo"] | None = None
    VERBOSE: bool = False
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    "VLLM_ROCM_USE_AITER_PAGED_ATTN": lambda: (
        os.getenv("VLLM_ROCM_USE_AITER_PAGED_ATTN",
    ...
    "VLLM_ROCM_USE_AITER_LINEAR": lambda: (
        os.getenv("VLLM_ROCM_USE_AITER_LINEAR",
                  "True").lower() in ("true", "1")
    ),
    ...
    "VLLM_ROCM_USE_AITER_RMSNORM": lambda: (
        os.getenv("VLLM_ROCM_USE_AITER_RMSNORM",
                  "True").lower() in ("true", "1")
    ),
    ...
    "VLLM_ROCM_USE_AITER_FP4_ASM_GEMM": lambda: (
        os.getenv("VLLM_ROCM_USE_AITER_FP4_ASM_GEMM",
                  "False").lower() in ("true", "1")
    ),
    # Whether to use aiter rope.
    # By default is disabled.
    "VLLM_ROCM_USE_TRITON_ROPE": lambda: (
        os.getenv("VLLM_ROCM_USE_TRITON_ROPE",
                  "False").lower() in ("true", "1")
    ),
    # Whether to use aiter triton fp8 bmm kernel
    # By default is enabled.
    "VLLM_ROCM_USE_AITER_FP8BMM": lambda: (
        os.getenv("VLLM_ROCM_USE_AITER_FP8BMM",
                  "True").lower() in ("true", "1")
    ),
    ...
    "VLLM_ROCM_CUSTOM_PAGED_ATTN": lambda: (
        os.getenv("VLLM_ROCM_CUSTOM_PAGED_ATTN",
                  "True").lower() in ("true", "1")
    ),
...
def compute_hash() -> str:
    ...
    factors = [environment_variables[key]()
               for key in environment_variables_to_hash]
